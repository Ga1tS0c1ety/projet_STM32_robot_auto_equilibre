...
static uint32_t i2c1_probe(uint8_t adresse_7_bits)
{
    uint32_t timeout;
...
static uint32_t i2c1_read_register(uint8_t adresse_7_bits,
                                   uint8_t registre,
                                   uint8_t *valeur)
{
    uint32_t timeout;
...
int main(void)
{
    ...
    uart_send_string("Activation de l'horloge I2C1...\r\n");


    i2c1_clock_enable();

    if (RCC_APB1ENR & RCC_APB1ENR_I2C1EN)
    {
        uart_send_string("Horloge I2C1 activee\r\n");
    }
    else
    {
        uart_send_string("Erreur : horloge I2C1 inactive\r\n");
    }


    i2c1_configure_input_clock();

    uart_send_string("CR2 FREQ = ");
    uart_send_uint(I2C1_CR2 & 0x3FUL);
    uart_send_string(" MHz\r\n");

    i2c1_configure_bus_speed();

    uart_send_string("CCR = ");
    uart_send_uint(I2C1_CCR & 0x0FFFUL);
    uart_send_string("\r\n");

    i2c1_configure_rise_time();

    uart_send_string("TRISE = ");
    uart_send_uint(I2C1_TRISE & 0x3FUL);
    uart_send_string("\r\n");
    ...
    {
        uart_send_string("I2C1 active\r\n");
    }
    else
    {
        uart_send_string("Erreur : I2C1 inactif\r\n");
    }
    ...
    {
        uart_send_string("Erreur : bus I2C occupe\r\n");
    }
    else if (resultat == 2)
    {
        uart_send_string("Erreur : START non genere\r\n");
    }
    else if (resultat == 3)
    {
        uart_send_string("Erreur : timeout pendant l'adressage\r\n");
    }
    else
    {
        uart_send_string("Erreur : aucune reponse a l'adresse 0x68\r\n");
    }
    ...
    erreur = i2c1_read_register(0x68, 0x75, &identifiant);
